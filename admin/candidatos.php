...
<body>
    ...
    <div class="container-fluid mt-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <?php if ($eleccionActual && $eleccionActual['estado'] !== 'publicada'): ?>
            <a href="?action=new&eleccion=<?= $eleccionId ?>" class="btn btn-primary"><i class="bi bi-plus-circle me-2"></i>Nuevo Candidato</a>
            <?php endif; ?>
        </div>
            ...
            <div class="col-md-3">
                <div class="card mb-3">
                    ...
                    <div class="list-group list-group-flush">
                        ...
                        <a href="?eleccion=<?= $e['id'] ?>" class="list-group-item list-group-item-action <?= $eleccionId == $e['id'] ? 'active' : '' ?>">
                            <?= $e['nombre'] ?>
                        </a>
                        <?php endforeach; ?>
            ...
            <div class="col-md-9">
                    ...
                    <div class="card-body py-2">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <span class="badge bg-<?= $eleccionActual['estado'] === 'activa' ? 'success' : 'secondary' ?> ms-2"><?= ucfirst($eleccionActual['estado']) ?></span>
                            </div>
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                ...
                                <tr class="<?= $c['activo'] ? '' : 'table-secondary' ?>">
                                    ...
                                    <td>
                                        <?php if ($eleccionActual['estado'] !== 'publicada'): ?>
                                        <a href="?action=edit&id=<?= $c['id'] ?>&eleccion=<?= $eleccionId ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></a>
                                        <a href="?action=toggle&id=<?= $c['id'] ?>&eleccion=<?= $eleccionId ?>" class="btn btn-sm btn-<?= $c['activo'] ? 'outline-warning' : 'outline-success' ?>"><i class="bi bi-<?= $c['activo'] ? 'eye-slash' : 'eye' ?>"></i></a>
                                        <a href="?action=delete&id=<?= $c['id'] ?>&eleccion=<?= $eleccionId ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('¿Eliminar?')"><i class="bi bi-trash"></i></a>
                                        <?php else: ?>
                                        <span class="text-muted"><i class="bi bi-lock"></i> Elección publicada</span>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php if (empty($candidatos)): ?>
                                <tr><td colspan="7" class="text-center text-muted py-4">No hay candidatos. <a href="?action=new&eleccion=<?= $eleccionId ?>">Agregar</a></td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="bi bi-<?= $id ? 'pencil' : 'plus-circle' ?> me-2"></i><?= $id ? 'Editar' : 'Nuevo' ?> Candidato</h5>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="?action=save<?= $id ? '&id=' . $id : '' ?>&eleccion=<?= $eleccionId ?>">
                            ...
                            <div class="row">
                                <div class="col-md-8 mb-3">
                                    ...
                                    <input type="text" name="nombre" class="form-control" value="<?= $candidatoEdit['nombre'] ?? '' ?>" required>
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label class="form-label">Orden</label>
                                    <input type="number" name="orden" class="form-control" value="<?= $candidatoEdit['orden'] ?? 0 ?>">
                                </div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Categoría</label>
                                <input type="text" name="categoria" class="form-control" value="<?= $candidatoEdit['categoria'] ?? '' ?>">
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="d-flex gap-2">
                                ...
                                <a href="?eleccion=<?= $eleccionId ?>" class="btn btn-secondary"><i class="bi bi-x-circle me-2"></i>Cancelar</a>
                            </div>
                        </form>
                    </div>
                </div>
